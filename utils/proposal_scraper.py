...
class ProposalScraper(object):
    ...
    def __init__(self, for_training=False,
                cycles_to_analyze=constants.scraper_cycles_to_analyze):
        """
        ...
        cycles_to_analyze: list
            List of proposal cycles, e.g. ['Cycle24', 'Cycle25']
        """
        self.is_nress = constants.is_nress
    ...
    def read_file(self, fname=None):
        ...
        with open(self.fname, 'r') as fobj:
            try:
                ...
                if (not self.is_nress) and ('AR' in self.text[1]):
                    self.archival = True
                else:
                    self.archival = False
    ...
    def extract_sections(self, verbose=False):
        ...
        # This is required because proposals might be missing sections, so when
        # looking for all data between two sections we need fine control over
        # what section is used to define the end of the current section
        next_idx = 1
    ...
    def extract_flist(self, cycle, flist=None):
        ...
        for f in tqdm.tqdm(flist, ascii=True, desc='Scraping Proposals'):
            self.read_file(fname=f)
            if self.text is None:
                continue

            if self.is_nress:
                self.section_data['All']=self.text
                self.proposal_label['Scientific Category']='NRESS'
                self.proposal_label['Scientific Keywords']='NRESS'
                self.write_training_data(
                    cycle=cycle,
                    training_sections=['All']
                    )
            else:
        ...
        for cycle in self._cycles_to_analyze:
            ...
            try:
                hand_classifications = glob.glob(
                    os.path.join(constants.path_output,
                                f"{cycle}_hand_classifications.txt")
                )[0]
            ...
            if flist is not None:
                ...
                LOG.info(f"No files found at {path} for Cycle {cycle}")
